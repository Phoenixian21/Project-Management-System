...
def update_data():
    query='update student set Members=%s,mobile=%s,email=%s,department=%s,Guide=%s,ProjectName=%s,ProjectDomain=%s,Research_Publication=%s,ResearchLink=%s,date=%s,time=%s where GroupNo=%s'
    mycursor.execute(query,(membersEntry.get(),MobileEntry.get(),emailEntry.get(),departmentEntry.get(),
...
def delete_student():
    ...
    query='delete from student where GroupNo=%s'
...
def search_data():
    query='select * from student where GroupNo=%s or members=%s or email=%s or mobile=%s or Department=%s or guide=%s or Research_Publication=%s'
    mycursor.execute(query,(GroupNoEntry.get(),membersEntry.get(),emailEntry.get(),MobileEntry.get(),departmentEntry.get(),guideEntry.get(),researchEntry.get()))
    studentTable.delete(*studentTable.get_children())
    fetched_data=mycursor.fetchall()
    for data in fetched_data:
        studentTable.insert('',END,values=data)
...
def add_data():
    if GroupNoEntry.get()=='' or membersEntry.get()=='' or MobileEntry.get()=='' or emailEntry.get()=='' or departmentEntry.get()=='' or guideEntry.get()=='' or researchEntry.get()=='' or domainentry.get()=='' or Projectentry.get()=='':
        messagebox.showerror('Error','All Fields are required',parent=screen)

    else:
        try:
            query='insert into student values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            mycursor.execute(query,(GroupNoEntry.get(),membersEntry.get(),MobileEntry.get(),emailEntry.get(),departmentEntry.get(),
...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor.execute(query)
            query='create table student(GroupNo varchar(30) not null primary key, Members varchar(80),mobile varchar(10),email varchar(30),department varchar(100),Guide varchar(20),ProjectName varchar(100),ProjectDomain varchar(100),Research_Publication varchar(20),ResearchLink varchar(20),date varchar(50), time varchar(50))'
            mycursor.execute(query)
        except:
            query='use projectmanagement'
            mycursor.execute(query)
...
addstudentButton=ttk.Button(leftFrame,text='Add Group Data',width=25,state=DISABLED,command=lambda :toplevel_data('Add Student','Add',add_data))
...
exportstudentButton=ttk.Button(leftFrame,text='Join Data',width=25,state=DISABLED,command=lambda :toplevel_data('Perform join','join',join_fun))
