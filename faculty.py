...
def joinuse():
    query = 'select groupdetails.groupid,groupdetails.members,groupdetails.roll,projectdetails.department,projectdetails.Guide,projectdetails.Research_Publication  from  groupdetails inner join projectdetails on groupdetails.groupid=projectdetails.groupid && projectdetails.Guide=%s && projectdetails.Research_Publication=%s'
    mycursor.execute(query, (guideEntry.get(),researchEntry.get()))
    con.commit()
...
def search_data():
    query='select * from projectdetails where Groupid=%s'
    mycursor.execute(query,(GroupNoEntry.get()))
    fetched_data=mycursor.fetchall()
    studentTable.delete(*studentTable.get_children())
    for data in fetched_data:
        studentTable.insert('',END,values=data)
